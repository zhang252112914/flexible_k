CUDA_VISIBLE_DEVICES=0,1 torchrun --nproc_per_node=2 \
--master_port=54198 \
main.py --do_train \
--num_thread_reader=4 \
--data_path /home/zyl/activitynet/captions \
--features_path /Data/datasets/ActivityNet/all_train_val_3fps \
--output_dir /home/zyl/flexible_k/output_an/output_topk_event_global_again \
--max_words 77 --max_frames 64 --batch_size_val 16 \
--datatype activity --feature_framerate 1 --coef_lr 1e-3 \
--slice_framepos 2 \
--loose_type --linear_patch 2d --sim_header meanP \
--pretrained_clip_name ViT-B/32 \
--post_process topk_event --post_cluster_centroids 16 \
--batch_size 16 --global_attn --global_again